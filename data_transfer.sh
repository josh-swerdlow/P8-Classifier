...
# Usage: ./data_transfer.sh <beg_seed_val> <end_seed_val>

if [[ ${1} = "help" ]]; then
    echo "Usage: ./data_transfer.sh <beg_seed_val> <end_seed_val>"
    echo "\tbeg seed val: First seed value"
    echo "\tend seed val: Last seed value\n"
    echo "Transfer all root, hdf5, and text files in the given range using scp."
    exit 0
fi
...
echo "Transfering Data for Seeds: ${seed_vals[@]}"
./move.sh ${seed_vals[@]}